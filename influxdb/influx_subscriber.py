...
class InfluxDBSubscriber:
    def __init__(self, bucket, measurements, interval=1):
        ...
        self.influx_client = InfluxDBClient(url=os.getenv("INFLUX_URL"), token=os.getenv("INFLUX_TOKEN"), org=os.getenv("INFLUX_ORG"))
        self.query_api = self.influx_client.query_api()

    def start(self):
        for measurement in self.measurements:
            thread = threading.Thread(target=self._fetch_data_loop, args=(measurement,), daemon=True)
    ...
    def _fetch_data_loop(self, measurement):
        while not self._stop_event.is_set():
            try:
                query = f'from(bucket: "{self.bucket}") |> range(start: -1m) |> filter(fn: (r) => r._measurement == "{measurement}") |> sort(columns: ["_time"], desc: true) |> limit(n: 1)'
                ...
                print(f'Error fetching data from InfluxDB for {measurement}: {e}')
                time.sleep(self.interval)
